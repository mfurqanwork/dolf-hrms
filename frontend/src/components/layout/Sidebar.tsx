...
export function Sidebar({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const visibleItems = getNavigationRoutes().filter(
    (item) => !item.adminOnly || user?.role !== "employee",
  );

  return (
    <aside className="flex h-full w-[17rem] flex-col bg-sidebar text-sidebar-foreground shadow-lg md:h-screen">
        ...
        <nav className="space-y-1.5 p-4">
          {visibleItems.map(({ path, icon: Icon, labelKey }) => (
            ...
            >
              ...
              {labelKey ? t(`common.${labelKey}`) : path}
            </NavLink>
          ))}
        </nav>
      </ScrollArea>
